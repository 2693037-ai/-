...
SHIELD_COL    = (60, 140, 255, 80)
BULLET_CORE   = (255, 255, 180)
BULLET_GLOW   = (255, 255, 80)
ENEMY_GLOW    = (255, 60,  60)
BOSS_ACCENT   = (200, 100, 255)
HUD_BG        = (8,  12,  30, 200)
...
def make_player_sprite(w=40, h=40):
    ...
    pygame.draw.polygon(surf, PLAYER_CORE, [
        (w//2, 2), (w//2-4, h//3+2), (w//2+4, h//3+2)
    ])
    pygame.draw.line(surf, PLAYER_CORE, (w//2, 2), (w//2, h*5//6), 2)
    pygame.draw.polygon(surf, (20, 60, 140), [
        (w//2-6, h//3), (0, h*3//4), (w//2-14, h*3//4)
    ])
    pygame.draw.polygon(surf, (20, 60, 140), [
        (w//2+6, h//3), (w, h*3//4), (w//2+14, h*3//4)
    ])
...
def make_boss_sprite(w=80, h=48):
    ...
    pygame.draw.polygon(surf, (60, 20, 100), [
        (8, 10), (0, h//2), (12, h*3//4)
    ])
    pygame.draw.polygon(surf, (60, 20, 100), [
        (w-8, 10), (w, h//2), (w-12, h*3//4)
    ])
...
def apply_augment(aug_id):
    ...
    global bullet_speed, bullet_straight, bullet_spread, AUTO_FIRE_INTERVAL, bullet_radius
    if aug_id == 'max_hp':
        player_max_hp += 1
...
def draw_text_shadowed(surface, text, font_, color, x, y, shadow_col=(0,0,0), offset=2):
    s = font_.render(text, True, shadow_col)
...
def draw_hud():
    ...
    if wave_state == 'playing':
        ...
        bar_y = 36
        ratio  = min(1.0, wave_kills / wave_kill_goal)
...
def draw_main_menu(tick):
    ...
    title_glow_alpha = int(40 + 60 * pulse)

    glow_s = pygame.Surface((SCREEN_WIDTH, 120), pygame.SRCALPHA)
    glow_s.fill((0, 60, 160, title_glow_alpha // 3))
    screen.blit(glow_s, (0, SCREEN_HEIGHT//2 - 180))
    ...
    ver = small_font.render("v2.0  Enhanced Edition", True, (60, 70, 90))
...
def draw_augment_screen(tick):
    ...
    for i, aug in enumerate(augment_choices):
        ...
        if hovered:
            ...
            shadow.fill((0, 0, 0, 0))
            pygame.draw.rect(shadow, (0, 0, 0, 80), (5, 5, CARD_W, CARD_H), border_radius=12)
            screen.blit(shadow, (cr_draw.x - 5, cr_draw.y))

        bg_col = (55, 55, 80) if hovered else (35, 35, 55)
        pygame.draw.rect(screen, bg_col, cr_draw, border_radius=10)

        border_col = aug['color']
        ...
        pygame.draw.rect(screen, border_col, cr_draw, border_w, border_radius=10)
        ...
        if hovered:
            ...
            bt = small_font.render("선택", True, (160, 160, 180))
...
def draw_wave_clear(timer, wave_num):
    ...
    if timer % 8 == 0 and timer < 80:
        for _ in range(3):
            cx = random.randint(SCREEN_WIDTH//5, SCREEN_WIDTH*4//5)
            cy = random.randint(SCREEN_HEIGHT//4, SCREEN_HEIGHT*3//4)
            col = random.choice([WAVE_GOLD, PLAYER_CORE, HOT_PINK, GREEN])
            spawn_explosion(cx, cy, col, 12)
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            running = False
            break  # ← QUIT 이벤트 즉시 루프 탈출

        if main_menu:
            if event.type == pygame.KEYDOWN or event.type == pygame.MOUSEBUTTONDOWN:
                main_menu = False
                reset_game()
                break  # ← 게임 시작 후 남은 이벤트 무시, 이벤트 루프 탈출
            continue
    ...
    if wave_state in ('playing', 'boss', 'wave_clear'):
        ...
        for enemy in enemies:
            ...
            etype  = enemy[2]
            if etype == 5:
                ew, eh = SPLIT_WIDTH, SPLIT_HEIGHT
            else:
                ew, eh = enemy_width, enemy_height
            ...
            ehp_max = max_hp_map.get(etype, 1)
            if ehp_max > 1 or enemy[3] < ehp_max:
